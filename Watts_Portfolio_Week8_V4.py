...
# 1. Create a list of Tuples that hold the characters and their frequencies. This stage creates a dictionary to start
# with, but then because we sort it, it is cast into a list of Tuples. This stage has a time complexity of O(n) as it
# loops over each character in the given phrase. It is important that this list is sorted in reverse order as we will
# unpack the Tuples from the end of the list first
...
def tally(phrase):
    ...
    # gives it a value of one, and if it is, it adds one to it's value.
    phrase_dict = {}
...
def huffman_encode(node, left=True, bin_string=''):
    ...
    # binary_string representation of the original phrase
    if type(node) is str:
...
def create_huffman_string(phrase, coding_dict):
    # Once we have the dictionary of characters and their codes, it is relatively simple to create the binary string
    # that represents the phrase. Vy looping over each character in the phrase, we find it's code in the dictionary
    # and add that code to the string that will be returned.
    coded_string = ""
    for char in phrase:
        coded_string += coding_dict[char]
    return coded_string
...
print("**********************************")
encoded_string = create_huffman_string(new_phrase, huffman_code)
